...
export default {
  ...
  for (const prx of prxList) {
    if (!groupedByCountry[prx.country]) {
      groupedByCountry[prx.country] = [];
    }
    groupedByCountry[prx.country].push(prx);
  }
  ...
  let html = `
    ...
    <body>
      <div class="sidebar">
        <h2>Menu</h2>
        <a href="/">🏠 Home</a>
        <a href="/sub">🌐 Proxy List</a>
        <a href="/tools">🛠️ Tools</a>
        <a href="/about">ℹ️ About</a>
      </div>
      <div class="content">
        <h1>Proxy List (Grouped by Country)</h1>
  ...
  // Tambahkan dropdown option berdasarkan country
  ...
  // Generate konten per negara
  for (const [country, proxies] of Object.entries(groupedByCountry)) {
    const flag = getFlagEmoji(country);
    html += `
  ...
  html += `
        <div class="footer">
          Generated by Cloudflare Worker • Host: ${APP_DOMAIN}
        </div>
      </div>
      ...
      <script>
        ...
        filter.addEventListener("change", () => {
          ...
          document.querySelectorAll(".country-block").forEach(block => {
            if (selected === "all" || block.dataset.country === selected) {
